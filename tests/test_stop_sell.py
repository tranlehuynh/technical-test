...
def test_stop_sell_price(trade_page, data, shared_data):
    time.sleep(3)
    ...
    shared_data["stop_loss_price"] = current_price * 1.1
    shared_data["take_profit_price"] = current_price * 0.2
    shared_data["trace_input_price_value"] = current_price * 0.9

    # Buy at market price
    time.sleep(2.5)
    noti_title = trade_page.stop_sell_price(shared_data["unit"], shared_data["stop_loss_price"],
                                           shared_data["take_profit_price"],
                                           shared_data["trace_input_price_value"])
    assert noti_title == "Stop Order Submitted", "Market order failed or popup not showed!"
...
@pytest.mark.dependency(depends=["test_stop_sell_price"])
def test_stop_edit_sell_price(trade_page, data, shared_data):
    shared_data["unit_edit"] = shared_data["unit"]
    shared_data["stop_loss_price_edit"] = shared_data["stop_loss_price"] - shared_data["stop_loss_price"] * 0.1
    shared_data["take_profit_price_edit"] = shared_data["take_profit_price"] * 0.9
    shared_data["trace_input_price_value_edit"] = shared_data["trace_input_price_value"] - shared_data[
        "trace_input_price_value"] * 0.2
    ...
    assert noti_title == "Stop Order Updated", "Market order failed or popup not showed!"
...
@pytest.mark.dependency(depends=["test_stop_edit_sell_price"])
